...
int FileSystem :: LseekFile(int fd, int iSize, int iFrom)
{
	if((fd < 0) || (iFrom > 2)) 
	{
		return -1;
	...
	if((UFDTArr[fd].ptrFileTable->iMode == READ) || (UFDTArr[fd].ptrFileTable->iMode == READ+WRITE))
	{
		if(iFrom == CURRENT)
		{
			if(((UFDTArr[fd].ptrFileTable->iReadOffset) + iSize) > UFDTArr[fd].ptrFileTable->ptrInode->iActualFileSize)
			{
				return -1;
			}
			
			if(((UFDTArr[fd].ptrFileTable->iReadOffset) + iSize) < 0)
	 		{
	 			return -1;
	 		}
			
			(UFDTArr[fd].ptrFileTable->iReadOffset) = (UFDTArr[fd].ptrFileTable->iReadOffset) + iSize;
		}
		else if(iFrom == START)
		{
			if(iSize > (UFDTArr[fd].ptrFileTable->ptrInode->iActualFileSize))
			{
		 		return -1;
		 	}
			...
			(UFDTArr[fd].ptrFileTable->iReadOffset) = iSize;
		}
		else if(iFrom == END)
		{
			if((UFDTArr[fd].ptrFileTable->ptrInode->iActualFileSize) + iSize > MAX_FILESIZE)
			{
				return -1;
			}
			...
			(UFDTArr[fd].ptrFileTable->iReadOffset) = (UFDTArr[fd].ptrFileTable->ptrInode->iActualFileSize) + iSize;
		}
	}
	else if(UFDTArr[fd].ptrFileTable->iMode == WRITE)
	{
		if(iFrom == CURRENT)
		{
			if(((UFDTArr[fd].ptrFileTable->iWriteOffset) + iSize) > MAX_FILESIZE)
			{
				return -1;
			}
			
			if(((UFDTArr[fd].ptrFileTable->iWriteOffset) + iSize) < 0)
			{
				return -1;
			}
			
			if(((UFDTArr[fd].ptrFileTable->iWriteOffset) + iSize) > (UFDTArr[fd].ptrFileTable->ptrInode->iActualFileSize)){ 
				(UFDTArr[fd].ptrFileTable->ptrInode->iActualFileSize) = (UFDTArr[fd].ptrFileTable->iWriteOffset) + iSize;
			}
			
			(UFDTArr[fd].ptrFileTable->iWriteOffset) = (UFDTArr[fd].ptrFileTable->iWriteOffset) + iSize;
		}
		else if(iFrom == START)
		{
			...
			if(iSize > (UFDTArr[fd].ptrFileTable->ptrInode->iActualFileSize)){
				(UFDTArr[fd].ptrFileTable->ptrInode->iActualFileSize) = iSize;
			}
			
			(UFDTArr[fd].ptrFileTable->iWriteOffset) = iSize;
		 }
		 else if(iFrom == END)
		 {
			if((UFDTArr[fd].ptrFileTable->ptrInode->iActualFileSize) + iSize > MAX_FILESIZE)
			...
			(UFDTArr[fd].ptrFileTable->iWriteOffset ) = (UFDTArr[fd].ptrFileTable->ptrInode->iActualFileSize) +iSize;
		}
	}
	return 0;
}
